...
function normalizeConversionSummary(summary: ConversionSummary): ConversionSummary {
  return {
    ...
    credits_spent: summary.credits_spent ?? 0,
    credits_after: summary.credits_after ?? 0,
  }
}
...
export function isConversionAuthError(error: unknown) {
  return error instanceof ConversionApiError && error.status === 401
}
